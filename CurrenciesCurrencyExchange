...
public class CurrenciesCurrencyExchange {

    exchangeApi exc = new exchangeApi();

    public void Country_currency_CurrencyExchange(String base_curr, String Country_name, int cardcode) throws SQLException, IOException {
        // North America
        if (Country_name.equals("United States")){
            Currency usa = Currency.getInstance("USD");
            String curr_code = usa.getCurrencyCode();
            exc.currency_exchange(base_curr, curr_code, cardcode);
        }
